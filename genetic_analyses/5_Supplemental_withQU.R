...
##without WI 
#calculate data points 
lat_points_red <- butternut_26pop_gendiv_geo_red$Mean_Lat
lat_points2_red <- lat_points_red^2

###Quadratic linear model
genstat_quad_model_red_lm <-lm(butternut_26pop_gendiv_geo_red$All_Rich ~ 
                             lat_points_red + lat_points2_red)

##Calculate summary documents 
genstat_quad_model_red_lm_sum <- summary(genstat_quad_model_red_lm)

##calculate y-axis values 
quad_values_red <- seq(min(butternut_26pop_gendiv_geo_red[,2]), max(butternut_26pop_gendiv_geo_red[,2]), 0.5)
quad_counts_red <- predict(genstat_quad_model_red_lm, list(lat_points_red=quad_values_red, lat_points2_red=quad_values_red^2))

##create r2 and p values
genstat_red_r2 <- genstat_quad_model_red_lm_sum$adj.r.squared
genstat_red_pvalue <- genstat_quad_model_red_lm_sum$coefficients[2,4]
genstat_red_rp <- vector('expression',2)
genstat_red_rp[1] <- substitute(expression(italic(R)^2 == MYVALUE), 
                            list(MYVALUE = format(genstat_red_r2,dig=3)))[2]
genstat_red_rp[2] = substitute(expression(italic(p) == MYOTHERVALUE), 
                           list(MYOTHERVALUE = format(genstat_red_pvalue, digits = 2)))[2]
